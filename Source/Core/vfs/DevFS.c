                               ...
                               const uint8_t *buffer, uint32_t size)
{
    ...
    switch (kind) {
        ...
        case DEVFS_KIND_INPUT_EVENT1:
            /* Output events (EV_LED, EV_REP) written back to an evdev node.
             * There are no LEDs to light and repeat is generated by the X
             * server, so accept and drop them. Failing the write made
             * xf86-input-evdev log "kbd0: Failed to set keyboard controls:
             * Input/output error" on every keyboard (re)configuration. */
            return true;
        case DEVFS_KIND_TTY:
            if (buffer != NULL) {
                                  ...
                                  uint64_t length, uint32_t nonblock)
{
    ...
    switch (kind) {
        ...
        case DEVFS_KIND_URANDOM:
        case DEVFS_KIND_RANDOM: {
            /* Registering dev_read for the whole driver sends every read() on
             * a /dev node here, and these used to fall into the default case:
             * read(/dev/urandom) was EFAULT. NSS seeds its RNG that way, so
             * softoken failed to initialize and Chromium died with
             * "FATAL:crypto/nss_util.cc:146 nss_error=-8023" right after its
             * first TLS handshake. `buffer` is a user pointer: fill a bounce
             * buffer and copy out. */
            uint8_t tmp[256];
            uint64_t done = 0u;
            if (length > (1ull << 25)) {
            ...
            while (done < length) {
                uint32_t n = (uint32_t)(length - done < sizeof(tmp) ?
                                        length - done : sizeof(tmp));
                (void)devfs_vfs_read_at(file, 0u, tmp, n);
                if (copy_to_user(buffer + done, tmp, n) != 0u) {
                    return done != 0u ? (int64_t)done : -14;
                }
                done += n;
            }
            return (int64_t)done;

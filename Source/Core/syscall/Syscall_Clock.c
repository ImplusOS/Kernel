...
/* Nanoseconds since boot -> {sec,nsec}. This single monotonic base is shared
 * by every MONOTONIC-family clock so CLOCK_MONOTONIC, CLOCK_MONOTONIC_RAW and
 * CLOCK_BOOTTIME never disagree with each other or drift relative to the
 * timerfd/futex/nanosleep code, which is driven off the same helper
 * (TODO_Chromium_LinuxABI.md section 3.8). There is no suspend state, so
 * BOOTTIME == MONOTONIC by construction.
 *
 * timer_monotonic_ns() rather than timer_ticks()/timer_hz(): the tick counter
 * is only as accurate as the assumed interrupt rate, and measured against the
 * HPET it runs ~14% fast, so every guest timeout was short by that much. The
 * HPET is what timer_monotonic_ns() uses when the platform has one, and the
 * kernel's own timing already trusts it. */
...
int64_t syscall_clock_getres(int32_t clk_id, uint64_t res_ptr)
{
    ...
    switch (clk_id) {
        ...
        case CLOCK_REALTIME_COARSE:
        case CLOCK_MONOTONIC_COARSE: {
            /* Report what clock_monotonic_now() can actually resolve. With an
             * HPET that is far finer than a timer tick; claiming the tick
             * period made callers that round their deadlines up to the
             * reported resolution (glibc's timer code among them) wait whole
             * ticks longer than asked. */
            ...
            kernel_timespec_t value = {
                .tv_sec = 0,
                .tv_nsec = hpet_is_available() ? 1000
                                               : (int64_t)(1000000000ULL / hz),
            };
